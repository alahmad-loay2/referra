...
const ConfirmReferral = () => {
  ...
  useEffect(() => {
    const confirmReferral = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/employee/referral/confirm/${referralId}`, {
          method: 'GET',
  ...
  if (expired) {
    return (
      <div className="confirm-referral-container">
        <p>This referral confirmation link has expired. Please contact the person who referred you for a new link.</p>
      </div>
    );
  }
